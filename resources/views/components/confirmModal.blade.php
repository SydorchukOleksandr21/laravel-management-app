...
<div id="confirmModal" class="fixed inset-0 flex items-center justify-center bg-black/30 hidden" onclick="closeModalIfClickedOutside(event)">
    <div class="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full" onclick="event.stopPropagation()">
        <h2 class="text-xl font-semibold mb-4">{{__('modal.delete.title')}}</h2>
        <p class="text-gray-600 mb-6">{{__('modal.delete.content')}}</p>
        <form id="confirmForm" method="POST">
            @csrf
            @method('DELETE')
            <div class="flex justify-end gap-4">
                <button type="button" class="btn btn-primary py-2" onclick="closeModal()">{{__('label.cancel')}}</button>
                <button type="submit" class="btn btn-error py-2">{{__('label.delete')}}</button>
            </div>
        </form>
    </div>
</div>
